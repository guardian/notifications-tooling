...
import {
	...
	useLatestPublishedContent,
} from './useLatestPublishedContent';
...
describe('useLatestPublishedContent', () => {
	it('fetches and maps the latest articles response', async () => {
		...
		const { result } = renderHook(() => useLatestPublishedContent(), {
			wrapper,
		});
		...
		act(() =>
			queryClient.removeQueries({ queryKey: latestPublishedContentQueryKey }),
		);
	});
});
