...
const statusDisplay: Record<NotificationSummary['status'], HistoryAlert['status']> = {
    accepted: 'Accepted',
    delivered: 'Sent',
...
export const mapNotificationToHistoryAlert = (
    ...
    const content = contentKey ? payload.data.content.items[contentKey] : undefined;
    if (!channel || !content) {
        return undefined;
    }

    const appPushAudience = appPush?.audience.items ?? [];
    const destinationIds = newsletter?.audience.items ?? appPushAudience.map(({ name }) => name);
