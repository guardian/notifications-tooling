...
const applyPreviewDocumentUpdates = (document: Document, draft: DraftEmail) => {
	...
	if (image) {
		image.setAttribute(
			'src',
			draft.imageUrl ? draft.imageUrl : DEFAULT_BREAKING_NEWS_IMAGE,
		);
	...
	const populateDraft = async () => {
		...
		try {
			...
			const nextDraft: DraftEmail = {
				...data,
				brazeEmail: data.brazeEmail
					? data.brazeEmail
					: draft.brazeEmail || DEFAULT_BRAZE_RECIPIENT_EMAIL,
			};
			setDraft(nextDraft);
