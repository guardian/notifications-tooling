...
import { fetchArticle, searchLatestArticles } from '@services';
...
type SearchLatestArticles = () => Promise<CapiSearchResult>;
...
const searchLatestArticlesFromCapi: SearchLatestArticles = async () => {
	const [endpoint, apiKey] = await Promise.all([
		getSSMParameter('CAPI_ENDPOINT'),
		getSSMParameter('CAPI_API_KEY'),
	]);

	return searchLatestArticles({
		endpoint,
		apiKey,
		timeoutMs: CAPI_REQUEST_TIMEOUT_MS,
		pageSize: 10,
...
export const createContentRouter = (
	...
	searchArticles: SearchLatestArticles = searchLatestArticlesFromCapi,
) =>
	Router()
		.post(
			'/articles/resolve',
		...
		.get(
			...
			async (req: Request, res: Response) => {
				try {
					return res.status(200).json(await searchArticles());
				} catch {
					return res
						.status(502)
						.json(
