...
type GuardianArticleRequestBody = {
	url?: unknown;
};
...
export const mapCapiContentToArticle = (
	...
	const headline = sanitizeGuardianTitle(
		firstNonEmpty(content.fields?.headline, content.webTitle),
	);
...
const fetchGuardianArticleHandler = async (
	req: Request<Record<string, never>, unknown, GuardianArticleRequestBody>,
	res: Response,
) => {
...
notificationsRouter.post('/guardian-article', fetchGuardianArticleHandler);
notificationsRouter.post('/braze-email', sendNotificationHandler);
